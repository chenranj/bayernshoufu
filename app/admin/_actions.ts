...
export async function createPlayer(formData: FormData) {
  ...
  if (photo && photo.size > 0) {
    photoPath = await uploadToBucket('players', photo);
  }
...
export async function updatePlayer(formData: FormData) {
  ...
  if (photo && photo.size > 0) {
    ...
    if (existing?.photo_path) {
      await removeFromBucket('players', existing.photo_path);
    }
  }
...
export async function createJersey(formData: FormData) {
  ...
  if (galleryPaths.length === 0) {
    flashError('/admin/jerseys', 'No image uploaded');
  }
  ...
  if (error || !jersey) {
    for (const p of galleryPaths) {
      await removeFromBucket('jerseys', p);
    }
